...
per questa macchina→Core): questo modulo serve a chiunque debba raggiungere
un host via TCP semplice (Node-RED su OPS, l'API di gaia_admin, ecc.), da
qualunque piattaforma. Stdlib-only apposta — nessuna dipendenza da
installare.
...
cross-directory, ogni agent è un'unità di deploy indipendente):
  ...
  minipc/installation/net_resolve.py   (questa copia, macchina touring/installazione)
